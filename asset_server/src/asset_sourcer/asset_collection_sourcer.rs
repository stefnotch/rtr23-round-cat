use asset_common::{asset_collection::AssetCollection, AssetRef};
...
pub struct AssetCollectionSourcer {}

impl AssetCollectionSourcer {
    fn is_asset_collection_file(path: &SourceFileRef) -> bool {
...
impl AssetSourcer for AssetCollectionSourcer {
    fn might_read(&self, path: &SourceFileRef) -> bool {
        Self::is_asset_collection_file(path)
    }

    fn create_assets(&self, import_request: CreateAssetInfo, asset_server: &mut AssetInserter) {
        if !Self::is_asset_collection_file(&import_request.file_ref) {
            return;
        }
        let mut imported_asset = Asset::<AssetCollection>::new(
