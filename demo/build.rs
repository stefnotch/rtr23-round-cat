...
fn main() {
    ...
    let root_path = PathBuf::from("../assets/shaders/");
    let paths = fs::read_dir(&root_path).unwrap();
    println!("cargo:rerun-if-changed={}", root_path.to_string_lossy());

    compile_shaders(paths, root_path, PathBuf::new(), out_dir);
}

fn compile_shaders(paths: fs::ReadDir, root_path: PathBuf, parent_path: PathBuf, out_dir: String) {
    for entry in paths {
